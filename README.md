...
src/
  ...
  shared/      デバウンス、半角カナ変換、検索語正規化、検索語の一時引き継ぎストレージ、メッセージ型
...
代わりに、アイコンクリック（`action.onClicked`）・右クリック検索のどちらも、`src/background/searchWindow.ts` の `openOrFocusSearchWindow()` を通じて同じ検索画面（`popup.html`）を独立した `windows.create()` のポップアップ型ウィンドウとして開きます。
...
- **検索の表記揺れについて（API/データ側の挙動、実データで確認済み）:**
  - `GET /api/banks?name=...` 等の検索は、`name`/`hira`/`kana`/`roma` に対する単純な部分一致で、半角・全角や大文字・小文字の正規化は行われません。英字を含む名称はデータ上、全角・大文字で格納されています（例: 銀行コード `0005` の `name` は `三菱ＵＦＪ`）。そのため、検索語中の半角英字は送信前に全角大文字へクライアント側で正規化しています（`src/shared/searchQuery.ts`）。数字は銀行コード/支店コード検索に使うため変換していません。
  - 多くの銀行の `name` は「銀行」等の業態種別を含みません（例: 銀行コード `0001` の `name` は「みずほ」、`0138` は「横浜」）。一方で信用金庫・農協等は「〇〇信金」「〇〇農協」のように種別を含む名称になっており、この扱いは業態によって一貫していません。これは検索アルゴリズムの不具合ではなくデータ側の命名規則によるものと考えられるため、拡張機能側で種別語を推測して自動的に除去するような処理は行わず、検索欄下にヒントを表示するに留めています。あいまい検索・正規化検索は `zengin-pl-api` 側の改善余地として今後検討の余地があります。
