...
src/
  ...
  shared/      デバウンス、半角カナ変換、検索語の一時引き継ぎストレージ、メッセージ型、ウィンドウ拡張の計算
...
### Chrome(MV3)のservice worker起動とコンテキストメニュー登録

Chrome の Manifest V3 サービスワーカーは非アクティブ時に頻繁に終了し、次のイベントで再起動されます。再起動のたびにトップレベルのコードは最初から実行されますが、`runtime.onInstalled` は「インストール・更新・ブラウザ更新」の時にしか発火しないため、それだけに頼って `contextMenus.create()` を呼んでいると、開発中の手動リロード等のタイミングでメニューが再登録されずに右クリック検索が反応しなくなることがあります。

これに対応するため、`registerContextMenu()`(内部で `removeAll()` してから `create()` するため冪等)を次の3箇所すべてで呼び出しています。

- バックグラウンドスクリプトのトップレベル(service worker起動のたびに必ず実行される)
- `runtime.onInstalled`
- `runtime.onStartup`
...
12. 小さめのウィンドウサイズで検索を行い、支店を選択して詳細情報が表示された際にスクロールなしで全項目が見えるようウィンドウが自動的に縦へ拡張されることを確認
13. Chromeで拡張機能を「更新」ボタンで手動リロードした直後でも、右クリック検索メニューが引き続き表示・動作することを確認（service worker再起動後のメニュー再登録の確認）
